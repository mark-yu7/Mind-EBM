...
for data_type in ["train", "valid", "test"]:
    ...
    for ii, d in tqdm(enumerate(data)):
        ...
        for e, idx in entities:
            ...
            input_ids_tensor = torch.tensor(input_id).to(f"cuda:{device_ids[0]}")
            # 半精度输入
            input_ids_tensor = input_ids_tensor.half()

            output = model.module.generate(input_ids_tensor, **generation_config)
            ...
            for i in range(entity_len+windows):
                ...
                sequences_tensor = sequences.to(f"cuda:{device_ids[0]}").half()
                output = model.module.generate(sequences_tensor, **generation_config)
